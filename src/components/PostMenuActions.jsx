...
const PostMenuActions = ({ post }) => {
  ...
  const { deletePost, savePost, featurePost, isSaveMutating, isFeatureMutating, isDeleteMutating } = usePostStore();
  ...
  const isSaved = authUser ? savedPosts?.posts?.some((p) => p._id === post._id) : false;
  ...
  const handleFeature = () => {
    featurePost(post, post.isFeatured, queryClient);
  };
    ...
    <div>
      ...
      {/* Feature Post Btn */}
      {
        isAdmin && (
          ...
          >
            ...
            <span>Feature</span>
            {isFeatureMutating && <span className="text-xs">(in progress)</span>}
          </div>
        )}
      ...
      {authUser && (
        <div className='flex items-center gap-2 py-2 text-sm cursor-pointer' onClick={handleDelete}>
          ...
          {isDeleteMutating && <span className='text-sm'>Deleting...</span>}
        </div>
      )}
    </div>
  );
